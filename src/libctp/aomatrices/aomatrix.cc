...
namespace votca { namespace ctp {
    ...
    void AOMatrix::Fill( AOBasis* aobasis, ub::vector<double> r, AOBasis* ecp ) {
        ...
        for ( vector<AOShell*>::size_type _row = 0; _row <  aobasis->_aoshells.size() ; _row++ ){
        //for (vector< AOShell* >::iterator _row = aobasis->firstShell(); _row != aobasis->lastShell() ; _row++ ) {
            ...
            for ( vector<AOShell*>::size_type _col = 0; _col <= _row ; _col++ ){
        ...
        for ( ub::matrix<double>::size_type _i=0; _i < _aomatrix.size1(); _i++){
            for ( ub::matrix<double>::size_type _j=0; _j < _i; _j++){
        ...
        // Copy stuff to fill lower triangular part
         for ( ub::matrix<double>::size_type _i=0; _i < this->_aomatrix.size1(); _i++){
            for ( ub::matrix<double>::size_type _j=0; _j <= _i; _j++){
    ...
    void AOMatrix::Print( string _ident){
        ...
        for ( ub::matrix<double>::size_type i =0; i< this->_aomatrix.size1(); i++){
            for ( ub::matrix<double>::size_type j =0; j< this->_aomatrix.size2(); j++){
       ...
       void AOMatrix3D::Print( string _ident){
        cout << "\n" << endl;
        for ( ub::matrix<double>::size_type i =0; i< this->_aomatrix[0].size1(); i++){
            for ( ub::matrix<double>::size_type j =0; j< this->_aomatrix[0].size2(); j++){
    ...
    void AOMatrix::XIntegrate(vector<double>& _FmT, const double& _T  ){
        ...
        if ( _T >= 10.0 ) {
            ...
            for (vector<double>::size_type m = 1; m < _FmT.size(); m++ ){
                _FmT[m] = (2*m-1) * _FmT[m-1]/(2.0*_T) - exp(-_T)/(2.0*_T) ;
            }
        }

        if ( _T < 1e-10 ){
           for ( vector<double>::size_type m=0; m < _FmT.size(); m++){
