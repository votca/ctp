...
void PolarBackground::Polarize(int n_threads = 1) {
    ...
    CTP_LOG(logDEBUG,*_log) << "System & Ewald parameters" << flush;
    CTP_LOG(logDEBUG,*_log) << "  o Real-space unit cell:      " << _a << " x " << _b << " x " << _c << flush;
    CTP_LOG(logDEBUG,*_log) << "  o Real-space c/o (guess):    " << _R_co << " nm" << flush;
    CTP_LOG(logDEBUG,*_log) << "  o na(max), nb(max), nc(max): " << _na_max << ", " << _nb_max << ", " << _nc_max << flush;
    CTP_LOG(logDEBUG,*_log) << "  o 1st Brillouin zone:        " << _A << " x " << _B << " x " << _C << flush;
    CTP_LOG(logDEBUG,*_log) << "  o Reciprocal-space c/o:      " << _K_co << " 1/nm" << flush;
    CTP_LOG(logDEBUG,*_log) << "  o R-K switching param.       " << _alpha << " 1/nm" << flush;
    CTP_LOG(logDEBUG,*_log) << "  o Unit-cell volume:          " << _LxLyLz << " nm**3" << flush;
    CTP_LOG(logDEBUG,*_log) << "  o LxLy (for 3D2D EW):        " << _LxLy << " nm**2" << flush;
    CTP_LOG(logDEBUG,*_log) << "  o kx(max), ky(max), kz(max): " << _NA_max << ", " << _NB_max << ", " << _NC_max << flush;    
    ...
    */
    ...
    if (!_do_restart) {
        ...
        for (sit1 = _bg_P.begin()+16; sit1 < _bg_P.end(); ++sit1) {
            ...
            CTP_LOG(logDEBUG,*_log) << "ID = " << pseg->getId() << " (" << seg->getName() << ") " << flush;
        ...
        CTP_LOG(dbg,log) << flush << "Induce to first order" << flush;
        for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
                (*pit1)->InduceDirect();
            }
        ...
        CTP_LOG(dbg,log) << flush;
        CTP_LOG(dbg,log) << "Restarting from checkpoint => "
    ...
    CTP_LOG(dbg,log) << "  o Setup real-space neighbours at iteration " << setup_nbs_iter << flush;
    CTP_LOG(dbg,log) << "  o Generate k-vectors at iteration " << generate_kvecs_iter << flush;
    
    int max_iter = iter+_max_iter;
    double epstol = 1e-3;
    for ( ; iter != max_iter; ++iter) {
        CTP_LOG(dbg,log) << flush;
        CTP_LOG(dbg,log) << "Iter " << iter << " started" << flush;
        
        // III.A Reset 2nd-order fields (FU)
        CTP_LOG(dbg,log) << "  o Reset fields (FU)" << flush;
        for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
                (*pit1)->ResetFieldU();
            }
        ...
        CTP_LOG(dbg,log) << "  o (Re-)generate induction fields" << flush;
        // (1) Real-space intramolecular contribution
        CTP_LOG(dbg,log) << "  o Real-space, intramolecular" << flush;
        for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
        ...
        if (!_do_use_cutoff) {
            ...
            CTP_LOG(dbg,log) << "  o Atomic SI correction" << flush;
            rms = 0.0;
            rms_count = 0;
            for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
                for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
        ...
        // III.C Induce again
        CTP_LOG(dbg,log) << "  o Induce again" << flush;
        for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
        ...
        CTP_LOG(dbg,log) << "  o Convergence check" << flush;
        bool converged = true;
        double maxdU = -1;
        double avgdU = 0.0;
        int baseN = 0;
        ...
        if (converged) {
        	_converged = true;
            CTP_LOG(dbg,log) << flush;
            CTP_LOG(dbg,log) << ":: Converged induction fields" << flush;
            break;
        }
    ...
    if (iter == max_iter) {
    	CTP_LOG(dbg,log) << flush;
    	CTP_LOG(dbg,log) << "Reached maximum number of iterations. Stop here." << flush;
    }
