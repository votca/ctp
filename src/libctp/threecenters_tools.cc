...
namespace votca {
    namespace ctp {
        ...
        void TCMatrix::Cleanup() {
            ...
            for (std::vector< ub::matrix<float> >::size_type _i = 0; _i < _matrix.size(); _i++) {
	  ...
	  //cout << "\n" << endl;
            for (int k = 0; k < this->mtotal; k++) {
                for (ub::matrix<float>::size_type i = 0; i < _matrix[1].size1(); i++) {
        ...
        void TCMatrix::Fill(AOBasis& _gwbasis, AOBasis& _dftbasis, ub::matrix<double>& _dft_orbitals) {
            ...
            for ( vector<AOShell*>::size_type _is= 0; _is <  _gwbasis._aoshells.size() ; _is++ ){
	  ...
	  //void TCMatrix::FillBlock(std::vector< ub::matrix<float> >& _block, AOShell* _shell, AOBasis& dftbasis, ub::matrix<double>& _dft_orbitals) {
            ...
            for (vector< AOShell* >::iterator _row = dftbasis.firstShell(); _row != dftbasis.lastShell(); _row++) {
                ...
                for (vector< AOShell* >::iterator _col = dftbasis.firstShell(); _col != dftbasis.lastShell(); _col++) {
                    ...
                    if (nonzero) {
                        ...
                        for (ub::matrix<double>::size_type _m_level = 0; _m_level < _temp.size1(); _m_level++) {
                            for (int _i_gw = 0; _i_gw < _shell->getNumFunc(); _i_gw++) {
            ...
            for (  std::vector< ub::matrix<float> >::size_type i=min; i < _matrix.size(); i++){
